...
function addAnimal(name, fee) {
    if (!name.trim()) {
        ...
        throw new Error("Enter a valid adoption fee");
    } else {
        animals.push(name.trim().toLowerCase());
        fees.push(Number(fee));
    } 
}
function getAdoptionFee(animalName) {
    let index = animals.indexOf(animalName.trim().toLowerCase());
    if (index === -1) {
        throw new Error(`Animal not found in records! for ${animalName} .`);
    }
    return fees[index];
}
// Main program
